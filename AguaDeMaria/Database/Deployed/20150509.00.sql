ALTER VIEW DailySummary
AS
SELECT 
	CAST(ISNULL(O.Date, ISNULL(D.Date,ISNULL(S.Date,ISNULL(P.DATE,null)))) as DateTime) as [Date] , O.RoundOrderQty, O.SlimOrderQty,
...
(SELECT 
	CONVERT(NVARCHAR(10),DR.DRDate, 110) AS [Date], 
	SUM(DRP.AmountPaid) as AmountPaid
FROM DeliveryReceipt DR
	INNER JOIN DeliveryReceiptPayment DRP on DR.DeliveryReceiptId = DRP.DeliveryReceiptId
GROUP BY CONVERT(NVARCHAR(10),DR.DRDate, 110)) DP ON DP.Date = D.Date
